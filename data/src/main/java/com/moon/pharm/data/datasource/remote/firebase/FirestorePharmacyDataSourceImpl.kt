...
class FirestorePharmacyDataSourceImpl @Inject constructor(
    ...
    override fun getPharmacyFromFirestore(placeId: String): Flow<PharmacyDTO> = callbackFlow {
        ...
        val registration = docRef.addSnapshotListener { snapshot, error ->
            ...
            if (dto != null) {
                ...
                close(Exception(ERROR_MSG_PHARMACY_NOT_FOUND))
            }
        }
        awaitClose { registration.remove() }
    }

    override suspend fun savePharmacyToFirestore(pharmacyDTO: PharmacyDTO) {
        pharmacyCollection.document(pharmacyDTO.placeId).set(pharmacyDTO).await()
    }
}