...
class UpdateNicknameUseCase(
    ...
    suspend operator fun invoke(newNickname: String): DataResourceResult<Unit> {
        ...
        return when (val userResult = userRepository.getUserOnce(userId)) {
            ...
            is DataResourceResult.Success -> updateNickname(userResult.resultData, newNickname)
        }
    }

    private suspend fun updateNickname(user: User, newNickname: String): DataResourceResult<Unit> = coroutineScope {
